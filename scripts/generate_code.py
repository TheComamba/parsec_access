...
fn get_masses(metallicity: &Metallicity) -> &'static [f64; {masses_size}] {{
    match metallicity {{
...
def collect_mass_filenames(archive_name):
    ...
    if len(files) != MASSES_SIZE:
        raise Exception(f"Expected {MASSES_SIZE} files in {dir_name}, found {len(files)}")
    return files
...
def generate_masses_constant(metallicity, masses):
    masses_str = f"const {metallicity_variant_name(metallicity)}_SORTED_MASSES: [f64; {MASSES_SIZE}] = ["
    for mass in masses:
        masses_str += f"{mass}, "
    masses_str += "];"
    return masses_str
